...
def get_buy_stock_payoff(stock_price, spot_price, many):
    return (stock_price-spot_price) * many

def get_sell_stock_payoff(stock_price, spot_price, many):
    return (spot_price - stock_price) * many

def plot_buy_stock_payoff(spot_price, price_range, many , all_payoffs):
    payoff = []
    for price in price_range:
        payoff.append(get_buy_stock_payoff(price, spot_price, many))

    all_payoffs.append(payoff)
    plt.plot(price_range, payoff, color='green')

def plot_sell_stock_payoff(spot_price, price_range, many , all_payoffs):
    payoff = []
    for price in price_range:
        payoff.append(get_sell_stock_payoff(price, spot_price, many))
...
def plot_total_payoff(payoffs: list, stock_price_range, option_count, is_stock: bool):
    total_payoff = []

    for i in range(len(payoffs[0])):
        sum = 0
        for _ in range(option_count+ is_stock):
            sum+=payoffs[_][i]
